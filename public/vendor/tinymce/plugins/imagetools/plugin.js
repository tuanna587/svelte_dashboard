...
(function() {
    ...
    var makeOpen = function(editor, imageUploadTimerState) {
        return function() {
            var getLoadedSpec = function(currentState) {
                // console.log('makeOpen currentState ', currentState)
                return {
                    title: 'Edit Image',
                    size: 'large',
                    body: {
            ...
            imgOpt.each(function(img) {
                getEditableImage(editor, img.dom).each(function(_) {
                    findBlob(editor, img.dom).then(function(blob) {
                        ...
                        // console.log('state', state, 'getLoadedSpec', getLoadedSpec(state));
    ...
    //LOGO WATERMARK
    var logoMaskOpen = function(editor, imageUploadTimerState) {
        // console.log('imageUploadTimerState', imageUploadTimerState);
        return function(){
            var getPanelLogoMask = function() {
                return {
            ...
            imgOpt.each(function(img) {
                getEditableImage(editor, img.dom).each(function(_) {
                    findBlob(editor, img.dom).then(function(blob) {
                        var state = createState(blob);
                        editor.windowManager.open(getPanelLogoMask(state));
                    });
                });
            });
        }
    ...
    var register = function(editor, imageUploadTimerState) {
        global$1.each({
            ...
            mceImageLogoMask: logoMaskOpen(editor, imageUploadTimerState),
    ...
    var register$1 = function(editor) {
        ...
        //anh tuan add tool set logo mask
        ...
        // //anh tuan add tool set logo mask
        // editor.ui.registry.addButton('logomask', {
        //     tooltip: 'Logo Mask',
        //     icon: 'logomask',
        //     onAction: cmd('mceImageLogoMask')
        // });
