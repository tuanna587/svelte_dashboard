...
<nav class="absolute top-0 left-0 w-full z-10 bg-transparent dark:bg-slate-900 md:flex-row md:flex-nowrap md:justify-start flex items-center p-4">
  <div class="w-full mx-autp items-center flex justify-between md:flex-nowrap flex-wrap md:px-5 px-3">
    <!-- Toggler -->
    <button class="cursor-pointer text-black dark:text-red-600 opacity-50 dark:opacity-80 px-3 py-1 text-xl leading-none bg-transparent rounded border border-solid border-transparent" type="button" on:click={() => toggleCollapseShow('bg-white m-2 py-3 px-6')}>
      <i class="fas fa-bars" />
    </button>
    <!-- Brand -->
    <a class="text-white dark:text-red-600 text-sm uppercase hidden lg:inline-block font-semibold" href="#pablo" on:click={(e) => e.preventDefault()}> Dashboard </a>
    <!-- Form -->
